...
export default function DriverSettings() {
  ...
  return (
    <div className="max-w-lg mx-auto px-5 py-8 pb-12">
      <Link to="/driver/profile" className="inline-flex items-center gap-1.5 text-sm text-muted hover:text-ink transition-colors mb-6">
        ← Profile
      </Link>
      <div className="font-mono text-xs text-muted tracking-wider mb-1">DRIVER</div>
      <h1 className="text-3xl font-bold tracking-tight mb-6">Settings.</h1>

      {/* Account */}
      <Section title="ACCOUNT">
        <RowLink to="/driver/profile/edit" icon="✏️" label="Edit profile" sub="Name, photo, bio, phone" />
        <RowLink to="/driver/profile/edit" icon="🚗" label="My vehicle" sub={
          user?.vehicle_make
            ? `${user.vehicle_make} ${user.vehicle_model || ''} · ${user.vehicle_size || ''}`
      ...
      <Section title="IDENTITY & KYC">
        ...
        <div className="px-5 py-3.5 flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <div className="text-xs font-mono text-muted mt-0.5 tracking-widest">{user?.license_plate || '—'}</div>
          </div>
          <span className="font-mono text-[10px] bg-green-50 text-green-700 px-2 py-0.5 rounded-md">VERIFIED</span>
        </div>
      </Section>
      ...
      <Section title="NOTIFICATIONS">
        <Toggle label="Booking updates"    sub="When your booking status changes"   storageKey="notif_bookings" />
        <Toggle label="Payment receipts"   sub="Confirmation after each payment"    storageKey="notif_payments" />
        <Toggle label="Review reminders"   sub="Remind me to review after a trip"   storageKey="notif_reviews"  />
        <Toggle label="Promotions"         sub="Special offers and new spots nearby" storageKey="notif_promos"  />
        <RowLink to="/driver/notifications" icon="🔔" label="View all notifications" />
      </Section>
      ...
      <Section title="DANGER ZONE">
        ...
        <RowButton icon="🗑️" label="Delete account" sub="Permanently removes your data" onClick={() => setShowDelete(true)} danger />
      </Section>
      ...
      <ConfirmDialog
        ...
        message="This permanently deletes your account, vehicle info, bookings, and reviews. This cannot be undone."
